...
{
   // 프리프로세싱 값을 출력하여 초기화 배열로 따로 뺄 수 도 있다.
   if (!preprocessed) {
      int a[5]; int ca[5];
      for (a[0] = 1; a[0] <= 5; a[0]++)
         for (a[1] = 1; a[1] <= 5; a[1]++)
   ...
   for (int c = 0; c < 1000000; c++) {
      a = b = 0;
      /*파싱 부분을 Loop Unrolling 을 사용하고
          A[c] 를 반복적으로 접근하는 것을 포인터를 사용해서 줄이면
         속도향상을 기대할 수 있다*/
      for (int i = 0; i < 5; i++) {
         a *= 5;
         a += (A[c][i][0] - '1');
      ...
      /* 이 주석을 풀어 1의 자리 파싱을 여기서 한다면(?) 시간이 크게 증가하는 것을 볼 수 있음 
      a = b = 0;
      for (int i = 0; i < 5; i++) {
         a *= 5;
         a += (A[c][i][1] - '1');
      ...
      */
      ...
      if (a1_score != b1_score) continue;
      if (a0_score == 1 && b0_score == 1 && a1_score == 1 && b1_score == 1) continue;  // 모두 우선순위가 5 인 경우
      ans--;
   }
   return ans;
}
