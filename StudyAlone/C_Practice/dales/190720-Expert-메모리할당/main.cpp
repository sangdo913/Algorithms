...
static char memory[400000000]; // 400 M
static char dummy[253011];
static char atbl[100000000]; // 100 M
...
static char* unit_ptr[4][10000000]; // 10M
...
static bool verify()
{
	...
	for (register int unit = 0; unit < 4; unit++)
		for (register int c = 0; c < unit_count[unit]; c++)
		{
			...
			{
				verifyCnt++;
				if ((ptr < memory) || (ptr + unit_size[unit] > memory + 1562500 * 256)) return false;
				if (memcmp(ptr, cmp_tbl[c % mod], unit_size[unit]) != 0) return false;
			}
		}
...
int main()
{
	...
	while (unit_total != 400000000) // 400M
	{
		register int unit = pseudo_rand() % 4;
		register int index;

		register char* ptr = alloc_block(atbl, unit_size[unit]);
