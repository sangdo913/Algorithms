...
void mergePictures(int N, int M, int K, char pictures[MAXN][MAX_PIECE_SIZE][MAX_PIECE_SIZE]) {
    ...
    for(rint i = 1; i < N; ++i){
        //DOWN
        unsigned int key = (pictures[i][0][0] << 28) + (pictures[i][0][1] << 24) + (pictures[i][0][2] << 20) + (pictures [i][0][3] << 16)
        + (pictures[i][1][0] << 12) + (pictures[i][1][1] << 8) + (pictures[i][1][2] << 4) + pictures[i][1][3];
        
        //RIGHT
        unsigned int key2 = (pictures[i][0][0] << 28) + (pictures[i][1][0] << 24) + (pictures[i][2][0] << 20) + (pictures[i][3][0] << 16)
        + (pictures[i][0][1] << 12) + (pictures[i][1][1] << 8) + (pictures[i][2][1] << 4) + pictures[i][3][1];

        //LEFT
        unsigned int key3 = (pictures[i][0][M-2] << 28) + (pictures[i][1][M-2] << 24) + (pictures[i][2][M-2] << 20) + (pictures[i][3][M-2] << 16)
        + (pictures[i][0][M-1] << 12) + (pictures[i][1][M-1] << 8) + (pictures[i][2][M-1] << 4) + pictures[i][3][M-1];

        //UP
        unsigned int key4 = (pictures[i][M-2][0] << 28) + (pictures[i][M-2][1] << 24) + (pictures[i][M-2][2] << 20) + (pictures [i][M-2][3] << 16)
        + (pictures[i][M-1][0] << 12) + (pictures[i][M-1][1] << 8) + (pictures[i][M-1][2] << 4) + pictures[i][M-1][3];

        unsigned int mask = key2;
        // for(int i = 0; i < 8; ++i){
        //     //cout << (mask & 0xf) << ' ';
        //     mask >>=4;
        // }
    ...
    while(fr != re){
        int now = myqueue[fr++];
        unsigned int key = (pictures[now][0][0] << 12) + (pictures[now][0][1] << 8) + (pictures[now][0][2] << 4) + pictures [now][0][3];
        unsigned int key2 = (pictures[now][0][0] << 12) + (pictures[now][1][0] << 8) + (pictures[now][2][0] << 4) + pictures[now][3][0];

        for(int j = 1; j < M; ++j){
            key = (key << 16) + (pictures[now][j][0] << 12) + (pictures[now][j][1] << 8) + (pictures[now][j][2] << 4) + pictures[now][j][3];
            INFO *cur = myhash[0][key%HASHSIZE].next;
            while(cur != &myhash[0][key%HASHSIZE]){
                ...
                if(setPicture(id, pos[now][0], nextY)){
                    ...
                    pos[id][1] = nextY;
                    myqueue[re++] = id;
                    //cout << cur->d << ' ' << id << '\t' << pos[id][0] << ' ' << pos[id][1] << endl;
                }
                NEXT:
                cur= cur->next;
            }
